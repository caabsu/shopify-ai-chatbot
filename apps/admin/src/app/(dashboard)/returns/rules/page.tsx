...
function conditionsToText(conditions: Record<string, unknown>, reasonOptions: ReasonOption[]): string {
  const parts: string[] = [];
  if (conditions.reason) {
    const label = reasonOptions.find((r) => r.value === conditions.reason)?.label || String(conditions.reason).replace(/_/g, ' ');
    parts.push(`Reason: ${label}`);
  }
...
export default function ReturnRulesPage() {
  ...
  async function loadReasons() {
    try {
      ...
      const data = await res.json();
      if (data.available_reasons && data.reason_labels) {
        const options: ReasonOption[] = [{ value: '', label: 'Any reason' }];
        for (const slug of data.available_reasons) {
          options.push({ value: slug, label: data.reason_labels[slug] || slug.replace(/_/g, ' ') });
        }
  ...
  async function movePriority(ruleId: string, direction: 'up' | 'down') {
    ...
    await Promise.all([
      fetch(`/api/returns/rules/${rules[idx].id}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ priority: swapPriority }),
      }),
      fetch(`/api/returns/rules/${rules[swapIdx].id}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
      ...
      {showForm && (
        ...
        >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label className="text-xs font-medium mb-1 block" style={{ color: 'var(--text-secondary)' }}>Rule Name</label>
              <input
                type="text"
                value={form.name}
            ...
            <div>
              <label className="text-xs font-medium mb-1 block" style={{ color: 'var(--text-secondary)' }}>Action</label>
              <select value={form.action} onChange={(e) => setForm({ ...form, action: e.target.value })} className="w-full text-sm rounded-lg px-3 py-2 focus:outline-none focus:ring-2" style={inputStyle}>
            ...
            <div>
              <label className="text-xs font-medium mb-1 block" style={{ color: 'var(--text-secondary)' }}>Reason (condition)</label>
              <select value={form.reason} onChange={(e) => setForm({ ...form, reason: e.target.value })} className="w-full text-sm rounded-lg px-3 py-2 focus:outline-none focus:ring-2" style={inputStyle}>
            ...
            <div>
              <label className="text-xs font-medium mb-1 block" style={{ color: 'var(--text-secondary)' }}>Max Amount ($, optional)</label>
              <input type="number" step="0.01" value={form.max_amount} onChange={(e) => setForm({ ...form, max_amount: e.target.value })} placeholder="e.g., 50.00" className="w-full text-sm rounded-lg px-3 py-2 focus:outline-none focus:ring-2" style={inputStyle} />
            </div>
            <div>
              <label className="text-xs font-medium mb-1 block" style={{ color: 'var(--text-secondary)' }}>Max Order Age (days, optional)</label>
              <input type="number" value={form.max_order_age_days} onChange={(e) => setForm({ ...form, max_order_age_days: e.target.value })} placeholder="e.g., 30" className="w-full text-sm rounded-lg px-3 py-2 focus:outline-none focus:ring-2" style={inputStyle} />
            </div>
            {form.action === 'auto_approve' && (
              <div>
                <label className="text-xs font-medium mb-1 block" style={{ color: 'var(--text-secondary)' }}>Resolution Type</label>
                <select value={form.resolution_type} onChange={(e) => setForm({ ...form, resolution_type: e.target.value })} className="w-full text-sm rounded-lg px-3 py-2 focus:outline-none focus:ring-2" style={inputStyle}>
            ...
            <div>
              <label className="text-xs font-medium mb-1 block" style={{ color: 'var(--text-secondary)' }}>Priority (lower = higher priority)</label>
              <input type="number" value={form.priority} onChange={(e) => setForm({ ...form, priority: e.target.value })} className="w-full text-sm rounded-lg px-3 py-2 focus:outline-none focus:ring-2" style={inputStyle} />
            </div>
          </div>

          <div className="flex items-center justify-end gap-2 mt-5">
            <button onClick={cancelForm} className="text-xs px-4 py-2 rounded-lg font-medium transition-colors" style={{ backgroundColor: 'var(--bg-tertiary)', color: 'var(--text-primary)', border: '1px solid var(--border-primary)' }}>
              Cancel
            </button>
            <button onClick={handleSave} disabled={saving || !form.name.trim()} className="flex items-center gap-1.5 text-xs px-4 py-2 rounded-lg font-medium text-white transition-colors disabled:opacity-50" style={{ backgroundColor: 'var(--color-accent)' }}>
      ...
      <div className="rounded-xl overflow-hidden" style={{ backgroundColor: 'var(--bg-primary)', border: '1px solid var(--border-primary)' }}>
        {rules.length === 0 ? (
          <div className="p-12 text-center">
            ...
            <p className="text-xs mt-1" style={{ color: 'var(--text-tertiary)' }}>Add a rule to automatically handle return requests</p>
          </div>
        ) : (
          <div className="divide-y" style={{ borderColor: 'var(--border-secondary)' }}>
            {rules.map((rule, idx) => {
              ...
              return (
                <div key={rule.id} className="px-4 py-3" style={{ opacity: rule.enabled ? 1 : 0.5 }}>
                  <div className="flex items-center justify-between">
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-1">
                        <span className="text-sm font-medium" style={{ color: 'var(--text-primary)' }}>{rule.name}</span>
                        <span className="text-[10px] font-medium px-2 py-0.5 rounded-full" style={{ backgroundColor: actionStyle.bg, color: actionStyle.text }}>
                          {actionStyle.label}
                        </span>
                        ...
                        <span className="text-[10px]" style={{ color: 'var(--text-tertiary)' }}>Priority: {rule.priority}</span>
                      </div>
                      <p className="text-xs" style={{ color: 'var(--text-secondary)' }}>
                        {conditionsToText(rule.conditions, reasonOptions)}
                      </p>
                    </div>
                    ...
                    <div className="flex items-center gap-1.5 flex-shrink-0">
                      ...
                      <button
                        ...
                        style={{ backgroundColor: rule.enabled ? 'var(--color-accent)' : 'var(--border-primary)' }}
                      >
                        <div className="w-4 h-4 bg-white rounded-full absolute top-1 transition-transform" style={{ left: rule.enabled ? '20px' : '4px' }} />
                      </button>

                      {/* Edit */}
                      <button onClick={() => openEditForm(rule)} className="p-1.5 rounded-lg transition-colors" style={{ color: 'var(--text-tertiary)' }}
