...
export default function TrackingPlaygroundPage() {
  ...
  useEffect(() => {
    function handleMessage(event: MessageEvent) {
      ...
      if (!type?.startsWith('otw:')) return;

      const newEvent: DebugEvent = {
        id: ++eventCounter,
        type,
        label: type.replace('otw:', ''),
        data,
        timestamp: Date.now(),
      };
  ...
  const eventColors: Record<string, { color: string; bg: string }> = {
    ...
    result: { color: '#a855f7', bg: 'rgba(168,85,247,0.10)' },
    ...
    <div className="flex flex-col h-[calc(100vh-2rem)] gap-4">
      ...
      <div className="flex items-center justify-between flex-shrink-0">
        <div>
          ...
          <p className="text-xs mt-0.5" style={{ color: 'var(--text-tertiary)' }}>
            Test the order tracking widget with debug information
          </p>
        </div>
        <div className="flex items-center gap-2">
          <a
            href={`${backendUrl}/widget/preview-tracking${brandQs ? `?${brandQs}` : ''}`}
            target="_blank"
            rel="noopener"
      ...
      <div className="flex-1 grid grid-cols-1 lg:grid-cols-[1fr_300px] gap-4 min-h-0">
        ...
        >
          <iframe
            key={iframeKey}
            src={`${backendUrl}/widget/preview-tracking?${[brandQs, 'debug=1'].filter(Boolean).join('&')}`}
        ...
        >
          ...
          <div className="mt-auto pt-3" style={{ borderTop: '1px solid var(--border-secondary)' }}>
            <p className="text-[11px] leading-relaxed" style={{ color: 'var(--text-tertiary)' }}>
              Listening for <code className="font-mono text-[10px]">otw:*</code> events from the
