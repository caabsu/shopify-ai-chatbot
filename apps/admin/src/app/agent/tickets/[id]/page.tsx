...
const PRIORITY_STYLES: Record<string, { bg: string; text: string }> = {
  ...
  high: { bg: 'rgba(249,115,22,0.12)', text: 'var(--color-warning)' },
  medium: { bg: 'rgba(59,130,246,0.12)', text: 'var(--color-info)' },
  low: { bg: 'rgba(156,163,175,0.12)', text: '#9ca3af' },
};

const STATUS_STYLES: Record<string, { bg: string; text: string }> = {
  open: { bg: 'rgba(59,130,246,0.12)', text: 'var(--color-info)' },
  pending: { bg: 'rgba(245,158,11,0.12)', text: 'var(--color-warning)' },
  resolved: { bg: 'rgba(34,197,94,0.12)', text: 'var(--color-success)' },
  closed: { bg: 'rgba(156,163,175,0.12)', text: '#9ca3af' },
};

const SOURCE_META: Record<string, { icon: typeof Mail; label: string; color: string }> = {
  email: { icon: Mail, label: 'Email', color: 'var(--color-info)' },
  form: { icon: FormInput, label: 'Form', color: 'var(--color-success)' },
  ...
  if (!data) {
    return (
      <div className="text-center py-12">
        ...
        <Link href="/agent/tickets" className="text-sm mt-2 inline-block" style={{ color: 'var(--color-accent)' }}>
          Back to inbox
        </Link>
      </div>
    );
