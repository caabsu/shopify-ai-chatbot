...
export function ReturnDetail({ id, backHref = '/returns' }: { id: string; backHref?: string }) {
  ...
  if (!data) {
    return (
      <div className="text-center py-12">
        ...
        <Link href={backHref} className="text-sm mt-2 inline-block" style={{ color: 'var(--color-accent)' }}>
          Back to returns
        </Link>
      </div>
    );
