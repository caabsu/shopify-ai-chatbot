...
export function createHeader(onMinimize: () => void, onClose: () => void, onReset?: () => void): HTMLElement {
  ...
  const title = state.headerTitle || 'Outlight';
  const subtitle = state.headerSubtitle || 'CONCIERGE';
  const logo = state.headerLogo || 'O';
  ...
  header.innerHTML = `
    <div class="aicb-header__info">
      ...
      <div class="aicb-header__titles">
        ...
        ${subtitle ? `<span class="aicb-header__subtitle">${subtitle}</span>` : ''}
      </div>
    </div>
