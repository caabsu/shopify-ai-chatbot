...
import type { OrderItem } from '../api/client.js';

export function createReturnsPortal(): HTMLElement {
  ...
  header.innerHTML = `
    ...
    <h2 class="wbd-returns__title">Start a return</h2>
    <p class="wbd-returns__subtitle">Changed your mind? No problem. Returns are free within 30 days.</p>
  `;
  ...
  reasonSection.innerHTML = `
    <div class="wbd-returns__row" style="margin-bottom:16px;">
      <div class="wbd-returns__field wbd-returns__field--full">
        ...
        <select class="wbd-returns__select" id="wbd-reason">
          ...
          <option value="doesnt_fit">Doesn't fit the space</option>
    ...
    <button class="wbd-returns__submit" id="wbd-submit-btn">Submit return request</button>
  ...
  successSection.innerHTML = `
    <div class="wbd-returns__success">
      ...
      <h3 class="wbd-returns__success-title">Return request submitted</h3>
      <p class="wbd-returns__success-text">We'll send you a return label and instructions within 2 business days.</p>
    </div>
  `;
  ...
  lookupBtn.addEventListener('click', async () => {
    ...
    try {
      ...
      lookupBtn.innerHTML = `<span class="material-symbols-outlined">search</span>Look up order`;
    }
  });
  ...
  function renderItems(orderItems: OrderItem[]) {
    ...
    for (const item of orderItems) {
      ...
      row.innerHTML = `
        ...
        <div class="wbd-returns__item-price">${item.currency} ${item.price}</div>
      `;
  ...
  submitBtn.addEventListener('click', async () => {
    ...
    try {
      await submitReturn({
        orderId,
        items: Array.from(selectedItems).map((id) => ({ itemId: id, reason })),
